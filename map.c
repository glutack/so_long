...
void	ft_redraw_exit(int y, int x, t_program *mlx)
{
	if (mlx->map_done[y - 1][x] == 'E')
		mlx_put_image_to_window(mlx->mlx, mlx->win, mlx->img.exit,
			mlx->x * 80, mlx->y * 80);
	else if (mlx->map_done[y + 1][x] == 'E')
		mlx_put_image_to_window(mlx->mlx, mlx->win, mlx->img.exit,
			mlx->x * 80, mlx->y * 80);
	else if (mlx->map_done[y][x - 1] == 'E')
		mlx_put_image_to_window(mlx->mlx, mlx->win, mlx->img.exit,
			mlx->x * 80, mlx->y * 80);
	else if (mlx->map_done[y][x + 1] == 'E')
		mlx_put_image_to_window(mlx->mlx, mlx->win, mlx->img.exit,
