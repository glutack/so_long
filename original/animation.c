...
static void	ft_animate_enemies(t_program *mlx)
{
	if (mlx->map.elap == 0)
	{
		if (mlx->map.eptr == mlx->img.paulinor2)
	...
	if (mlx->map.elap == 2)
	{
		if (mlx->map.eptr == mlx->img.paulinol2)
			mlx->map.eptr = mlx->img.paulinol3;
		else if (mlx->map.eptr == mlx->img.paulinol3)
			mlx->map.elap = 3;
	}
	else if (mlx->map.elap == 3)
	{
		if (mlx->map.eptr == mlx->img.paulinol3)
	...
	while (mlx->y < mlx->map.winy)
	{
		while (mlx->map_done[mlx->y][mlx->x] != '\0')
		{
			if (mlx->map_done[mlx->y][mlx->x] == 'C')
				...
				mlx_put_image_to_window(mlx->mlx, mlx->win, mlx->map.pptr,
					mlx->x * 80, mlx->y * 80);
			mlx->x++;
		}
		mlx->y++;
		...
		while (mlx->map_done[mlx->y][mlx->x] != '\0')
		{
			if (mlx->map_done[mlx->y][mlx->x] == 'B')
			{
				if (mlx->map_done[mlx->y][mlx->x - 1] == '1')
						mlx->map.elap = 0;
				else if (mlx->map_done[mlx->y][mlx->x + 1] == '1')
						mlx->map.elap = 2;
				if (mlx->map.elap <= 1)
				{
